...
class Zomato:
    ...
    def get_restaurant(self, restaurant_ID):
        ...
        restaurant_details.update({"currency" : a['currency']})
        restaurant_details.update({"user_rating" : a['user_rating']['aggregate_rating']})
        restaurant_details.update({"is_delivering_now" : a['is_delivering_now']})
        restaurant_details.update({"has_table_booking" : a['has_table_booking']})
        restaurant_details.update({"cuisines" : a['cuisines']})
        restaurant_details = DotDict(restaurant_details)
        return restaurant_details


    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", limit=0):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&start=" + str(limit) + "&count=20" + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines), headers=headers).content).decode("utf-8")
